...
const ModelComparison = ({ selectedModelId, inputTokens, outputTokens }) => {
  ...
  return (
    <div className="bg-[#FFFDF0] border-4 border-black rounded-[8px] p-5 md:p-6 shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] w-full overflow-hidden">
      <h3 className="text-xl md:text-2xl font-black uppercase mb-6 inline-block border-b-4 border-black pb-1">
        Cost Comparison
      </h3>
      
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        {/* Selected Model Card */}
        <div className="bg-[#B3DDF2] border-4 border-black rounded-lg p-4 shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] flex flex-col h-full relative">
          ...
          <div className="font-black text-lg leading-tight mb-4">{selectedModel.name}</div>
          
          <div className="mt-auto space-y-2 text-sm border-t-2 border-black/20 pt-4">
        ...
        {similarModels.map(model => (
          <div key={model.id} className="bg-white border-4 border-black rounded-lg p-4 shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] flex flex-col h-full">
            <div className="text-xs font-black uppercase tracking-widest text-black/60 mb-1">{model.provider}</div>
            <div className="font-black text-lg leading-tight mb-4">{model.name}</div>
